...
{
    public class TransferIndianaEventsToBigQueryService
        ...
        protected override async Task<List<IndianaEventDto>> GetIndianaEventsAsync(string locationId, DateTime day)
        {
            ...
            try
            {
                ...
                if (await reader.ReadAsync())
                {
                    ...
                    events = JsonConvert.DeserializeObject<List<IndianaEventDto>>(json);
                    events.ForEach(e => e.LocationIdentifier = locationId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error reading data for {LocationId} on {Date}", locationId, day);
            }
